...
void main_DetectWater(void)
{
  ...
  /* Initialize function input argument 'gswater'. */
  gswater = c_argInit_UnboundedxUnbounded_r();
  /* Call the entry-point 'DetectWater'. */
  emxInitArray_uint8_T(&water, 2);
  emxInitArray_uint8_T(&waterAll, 2);
  argInit_1x2_real_T(dv);
  DetectWater(dv, mask, nir, NDVI, psnow, gswater, water, waterAll);
  emxDestroyArray_real32_T(gswater);
