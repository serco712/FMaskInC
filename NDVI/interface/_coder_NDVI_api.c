...
void NDVI_api(const mxArray *const prhs[2], const mxArray **plhs)
{
  ...
  emxInit_real32_T(&st, &red);
  red->canFreeData = false;
  emlrt_marshallIn(&st, emlrtAlias(prhs[0]), "red", red);
  emxInit_real32_T(&st, &nir);
  nir->canFreeData = false;
  emlrt_marshallIn(&st, emlrtAlias(prhs[1]), "nir", nir);
  /* Invoke the target function */
  emxInit_real32_T(&st, &ndvi);
  NDVI(red, nir, ndvi);
  emxFree_real32_T(&st, &nir);
  emxFree_real32_T(&st, &red);
  /* Marshall function outputs */
  ndvi->canFreeData = false;
  *plhs = emlrt_marshallOut(ndvi);
...
/* End of code generation (_coder_NDVI_api.c) */
